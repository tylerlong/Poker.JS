...
(function () {
  if (window.CanvasRenderingContext2D) {
    ...
    const P = 13;
    const R = document;
    const S = CanvasRenderingContext2D.prototype;
    const T = Math.PI;
    const symbolPath = {
    ...
    const pc = function (size) {
      const canvas = R.createElement('canvas');
      canvas.height = size;
    ...
    S.drawPokerCard = function (x, y, size, suit, point) {
      const Z = this;
      const X = Z.fillPokerSymbol;
      const Y = Z.drawPokerCrown;
      suit = fixSuit(suit);
    ...
    S.svgCurve = function (x, y, size, svgPath) {
      ...
      for (pathNumber in svgPathArray) {
        ...
        svgCmd = pathArray[0];
        svgCmd === 'M'
          ? Z.moveTo(ax(pathArray[1]), ay(pathArray[2]))
          : svgCmd === 'L'
            ? Z.lineTo(ax(pathArray[1]), ay(pathArray[2]))
            : svgCmd === 'H'
              ? Z.lineTo(ax(pathArray[1]), relativeY)
              : svgCmd === 'V'
                ? Z.lineTo(relativeX, ay(pathArray[1]))
                : svgCmd === 'C'
                  ? Z.bezierCurveTo(
                      ax(pathArray[1]),
                      ay(pathArray[2]),
                  ...
                  : svgCmd === 'Q' &&
                    Z.quadraticCurveTo(ax(pathArray[1]), ay(pathArray[2]), ax(pathArray[3]), ay(pathArray[4]));
      }
      Z.closePath();
    };
    ...
    S.drawPokerSymbol = function (x, y, size, symbol) {
      ...
      if (symbolPath[symbol]) this.svgCurve(x, y, size, symbolPath[symbol]);
    };
    ...
    window.Poker = {
      getCardImage: function (size, suit, point) {
        const image = R.createElement('img');
